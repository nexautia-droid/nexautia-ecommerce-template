...
export default function AdminPanel({ locale }: { locale: Locale }) {
  ...
  async function saveProduct(event: FormEvent) {
    ...
    const translations = [editing.es, editing.ca].map((item) => ({ ...item, product_id: productId }));
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-content">
      ...
      {editing ? <form className="product-form" onSubmit={saveProduct}>
        <div className="form-grid compact"><label>{t.internal}<input value={editing.internal_name} onChange={(e) => setEditing({ ...editing, internal_name: e.target.value })} required/></label><label>{t.category}<select value={editing.category_id} onChange={(e) => setEditing({ ...editing, category_id: e.target.value })}><option value="">{t.none}</option>{categories.map((category) => <option value={category.id} key={category.id}>{category.internal_name}</option>)}</select></label><label>{t.status}<select value={editing.status} onChange={(e) => setEditing({ ...editing, status: e.target.value as Product["status"] })}><option value="draft">{t.draft}</option><option value="active">{t.active}</option><option value="archived">{t.archived}</option></select></label><label>{t.sku}<input value={editing.sku} onChange={(e) => setEditing({ ...editing, sku: e.target.value })} required/></label><label>{t.price}<input type="number" min="0" step="0.01" value={editing.price} onChange={(e) => setEditing({ ...editing, price: e.target.value })} required/></label><label>{t.stock}<input type="number" min="0" step="1" value={editing.stock} onChange={(e) => setEditing({ ...editing, stock: e.target.value })}/></label></div>
        {(["es", "ca"] as Locale[]).map((language) => <fieldset key={language}><legend>{language === "es" ? t.spanish : t.catalan}</legend><div className="form-grid"><label>{t.name}<input value={editing[language].name} onChange={(e) => { const name = e.target.value; updateTranslation(language, "name", name); if (!editing[language].slug) updateTranslation(language, "slug", slugify(name)); }} required/></label><label>{t.slug}<input value={editing[language].slug} onChange={(e) => updateTranslation(language, "slug", slugify(e.target.value))} required/></label><label className="wide">{t.short}<input value={editing[language].short_description ?? ""} onChange={(e) => updateTranslation(language, "short_description", e.target.value)}/></label><label className="wide">{t.description}<textarea value={editing[language].description ?? ""} onChange={(e) => updateTranslation(language, "description", e.target.value)}/></label><label>{t.seoTitle}<input value={editing[language].seo_title ?? ""} onChange={(e) => updateTranslation(language, "seo_title", e.target.value)}/></label><label>{t.seoDescription}<textarea value={editing[language].seo_description ?? ""} onChange={(e) => updateTranslation(language, "seo_description", e.target.value)}/></label></div></fieldset>)}
