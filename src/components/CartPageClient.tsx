...
export default function CartPageClient({ lang }: { lang: Locale }) {
  ...
  return <main className="cart-page">
    <header className="detail-header"><Link href={`/${lang}`}><Image src={publicAsset("/brand/logo.svg")} width={204} height={60} alt="Nexautia"/></Link><Link href={`/${lang}`}>{lang === "es" ? "Seguir comprando" : "Continuar comprant"} &larr;</Link></header>
    <section className="cart-content"><p className="eyebrow">Nexautia / E-commerce</p><h1>{lang === "es" ? "Tu cesta" : "La teva cistella"}</h1>
      {items.length === 0 ? <div className="empty-cart"><p>{lang === "es" ? "Todavía no has añadido ningún producto." : "Encara no has afegit cap producte."}</p><Link className="button primary" href={`/${lang}#productos`}>{lang === "es" ? "Ver productos" : "Veure productes"}</Link></div> : <>
        <div className="cart-list">{items.map((item) => <article className="cart-row" key={item.slug}><div><h2>{item.name}</h2><p>{item.price} EUR</p></div><div className="quantity"><button onClick={() => change(item.slug, item.quantity - 1)} aria-label={lang === "es" ? "Restar" : "Restar-ne"}>-</button><span>{item.quantity}</span><button onClick={() => change(item.slug, item.quantity + 1)} aria-label={lang === "es" ? "Sumar" : "Afegir-ne"}>+</button></div><strong>{item.price * item.quantity} EUR</strong><button className="remove" onClick={() => change(item.slug, 0)}>{lang === "es" ? "Eliminar" : "Eliminar"}</button></article>)}</div>
